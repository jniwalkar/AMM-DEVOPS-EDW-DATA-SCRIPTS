...
cleanup_old_data() {
    log "${BLUE}Cleaning up data older than $DATA_RETENTION_MONTHS months...${NC}"

    local directories=$(aws s3 ls s3://$BUCKET/ | grep "PRE" | awk '{print $2}' | sed 's/\///g')

    if [ -z "$directories" ]; then
        log "${YELLOW}No directories for cleanup${NC}"
        return 0
    fi
    ...
    log "${GREEN}Cleanup done. Deleted $deleted_count directories${NC}"
}

create_monthly_directory() {
    log "${BLUE}Creating monthly directory: $MONTH_YEAR${NC}"
    aws s3api put-object --bucket "$BUCKET" --key "$MONTH_YEAR/" >/dev/null 2>&1
    [ $? -eq 0 ] && log "${GREEN}Directory created${NC}" || log "${RED}Failed${NC}"
}
...
check_environment() {
    command -v aws >/dev/null 2>&1 || { log "${RED}AWS CLI not found${NC}"; exit 1; }
    aws s3 ls s3://$BUCKET >/dev/null 2>&1 || { log "${RED}Cannot access S3${NC}"; exit 1; }
    log "${GREEN}S3 accessible${NC}"

    command -v lftp >/dev/null 2>&1 && log "${GREEN}lftp available${NC}" || log "${YELLOW}lftp not found - SFTP will be skipped${NC}"
...
QUERY2='SELECT grp.label AS groupname, grp.groupid, stat.statlabel, latest.value, count(*) AS num_gateways FROM im_group grp JOIN im_node node ON grp.groupid = node.groupid JOIN im_lateststatitem latest ON latest.nodeid = node.nodeid JOIN im_stat stat ON latest.statid = stat.statid WHERE grp.groupid IS NOT NULL AND node.groupid <> 0 GROUP BY grp.groupid, grp.label, stat.statlabel, latest.value ORDER BY grp.label, grp.groupid, stat.statlabel, latest.value;'
...
create_excel_from_tsv() {
    ...
    #"$VENV_DIR/bin/python3" "$PYTHON_SCRIPT" "$tsv_dir" "$excel_output" "$server_name" 2>&1 | while read line; do
    #    log "${BLUE}  PY: $line${NC}"
    #done
...
upload_to_sftp() {
    ...
    if ! command -v lftp &> /dev/null; then
        ...
        SERVER_SFTP_STATUS[$server_name]="SKIPPED"
        return 0
    fi
...
process_server() {
    ...
    [ $? -ne 0 ] && { log "${RED}S3 failed${NC}"; SERVER_STATUS[$server_name]="FAILED"; SERVER_ERRORS[$server_name]="S3 upload failed"; rm -rf "$tsv_dir" "$local_tar" "$excel_file" "$script_path" "$output_file"; return 1; }

    local s3_path=$(echo "$s3_result" | cut -d'|' -f1)
    local size=$(echo "$s3_result" | cut -d'|' -f2)

    # Upload to SFTP
    upload_to_sftp "$excel_file" "$server_name"

    # Set status
    if [ $query_nodata -eq 3 ]; then
    ...
    if [ "${SERVER_SFTP_STATUS[$server_name]}" == "SUCCESS" ]; then
        SERVER_FILES[$server_name]="S3: $s3_path ($size)
        SFTP: ${SFTP_HOST}${SFTP_REMOTE_DIR}/$(basename $excel_file)"
    else
        SERVER_FILES[$server_name]="S3: $s3_path ($size)
        SFTP: ${SERVER_SFTP_STATUS[$server_name]}"
    fi
    ...
    for srv in $(echo "${!SERVERS[@]}" | tr ' ' '\n' | sort); do
        ...
        case "$status" in
            SUCCESS)
                ...
                echo "${SERVER_FILES[$srv]}" >> "$SUMMARY_FILE"
                ;;
            NO_DATA)
                echo "Result: Queries OK but no data" >> "$SUMMARY_FILE"
                echo "Reason: ${SERVER_ERRORS[$srv]}" >> "$SUMMARY_FILE"
                [ -n "${SERVER_FILES[$srv]}" ] && echo "Files: ${SERVER_FILES[$srv]}" >> "$SUMMARY_FILE"
                ;;
            FAILED)
...
EOF
    ...
    #command -v mail >/dev/null 2>&1 && cat "$SUMMARY_FILE" | mail -s "MySQL Dump - $MONTH_YEAR" "$SUMMARY_EMAIL" && log "${GREEN}Email sent${NC}" || log "${YELLOW}No mail${NC}"
